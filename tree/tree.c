...
int deleteChildren(Tree* tree, void* elementToBeDelete){
    ...
    DoubleList list;
    TreeNode* root = (TreeNode*)(tree->root);
    if(NULL != getTreeNode(root->children,elementToBeDelete,tree->cmp)){
        temp = getTreeNode(root->children,elementToBeDelete,tree->cmp);
        list = temp->parent->children;
        return deleteNode(&list,0);
    }
    return 0;
}
